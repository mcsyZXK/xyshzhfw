  ...
  <div class="admin-page">
    ...
    <div class="table-section">
      <el-card class="table-card" shadow="never">
        <el-table :data="forumList" v-loading="loading" style="width: 100%">
          <el-table-column prop="forumName" label="标题" min-width="200">
            <template #default="{ row }">
              <div class="title-cell">
                <span class="title-emoji">📝</span>
                <span class="title-text">{{ row.forumName }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="yonghuName" label="发布者" width="120" align="center">
            <template #default="{ row }">
              <div class="user-cell">
                <span>👤</span>
          ...
          <el-table-column label="状态" width="100" align="center">
            <template #default="{ row }">
              <el-tag :type="row.isTop === 1 ? 'success' : 'info'" effect="plain">
                {{ row.isTop === 1 ? '📌 置顶' : '📄 普通' }}
              </el-tag>
            </template>
          </el-table-column>
...
import { getForumList, deleteForum } from '@/api/forum'
...
const handleSearch = async () => {
  ...
  try {
    ...
    if (searchForm.forumName) params.forumName = searchForm.forumName

    const res = await getForumList(params)
    if (res && res.data) {
      forumList.value = res.data.list || res.data || []
      total.value = res.data.total || forumList.value.length
    }
  } catch (error) {
...
.page-header {
  ...
  box-shadow: 8px 8px 0 rgba($accent-blue, 0.3);
  position: relative;
  overflow: hidden;
