...
//            ServletContext servletContext = ContextLoader.getCurrentWebApplicationContext().getServletContext();
            ...
            if (dictionaryMap == null || dictionaryMap.isEmpty()) {
                dictionaryMap = initDictionaryMap();
                servletContext.setAttribute("dictionaryMap", dictionaryMap);
            }
     ...
     * 初始化字典缓存
     */
    private Map<String, Map<Integer, String>> initDictionaryMap() {
        List<DictionaryEntity> dictionaryEntities = this.selectList(new EntityWrapper<DictionaryEntity>());
        Map<String, Map<Integer, String>> map = new HashMap<>();
        if (dictionaryEntities == null || dictionaryEntities.isEmpty()) {
            return map;
        }
        for (DictionaryEntity d : dictionaryEntities) {
            Map<Integer, String> valueMap = map.get(d.getDicCode());
            if (valueMap == null) {
                valueMap = new HashMap<>();
            }
            valueMap.put(d.getCodeIndex(), d.getIndexName());
            map.put(d.getDicCode(), valueMap);
        }
        return map;
    }
